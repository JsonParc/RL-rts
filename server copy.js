...
function issueSquadMoveOrder(squad, targetX, targetY) {
  ...
  const formingUpUntil = Date.now() + 60000;
  ...
  socket.on('createSquad', (data) => {
    ...
    if (aliveUnits.length >= 2) {
      ...
      fPositions.forEach(({ unit }) => {
        ...
        unit.attackTargetType = null;
        unit.angle = avgAngle;
        unit.squadDesiredAngle = avgAngle;
        unit.squadDesiredAngleUpdatedAt = Date.now();
        unit.formingUp = true;
        unit.formingUpUntil = formingUpUntil;
        resetNavalAvoidanceState(unit);
      ...
      squad.lastFormationRetargetAt = 0;
      refreshSquadFormationMovementTargets(squad, aliveUnits, Date.now(), true);
    }
...
function updateGame(deltaTime) {
  ...
  // Per-tick squad management: track individual unit navigation without overriding low-level movement
  gameState.squads.forEach((squad, squadId) => {
    if (!squad) return;
    const units = getSquadAliveUnits(squad);
    ...
    refreshSquadFormationMovementTargets(squad, units, now, anchorChanged);

    // Squad state only tracks whether any member is still navigating to a formation slot.
