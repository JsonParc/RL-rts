...
// fogLayerCanvas is a gridSize×gridSize pixel canvas drawn once per fog update
...
// Initialize battleship images on load
...
window.addEventListener('keydown', (e) => {
    ...
    // G키 자원 채집 기능 제거됨 - 발전소가 자동으로 에너지 생산
    ...
    if (selectedBuildings.length > 0) {
        ...
        if (building.type === 'missile_silo' && building.buildProgress >= 100 && building.userId === gameState.userId) {
            ...
            let queueIconsHtml = missileQueue.map((item, idx) => {
                const isFirst = idx === 0;
                return `<span style="display:inline-block;width:24px;height:24px;text-align:center;line-height:24px;background:${isFirst ? 'rgba(206,147,216,0.3)' : 'rgba(255,255,255,0.1)'};border:1px solid ${isFirst ? '#ce93d8' : '#555'};border-radius:3px;font-size:14px;" title="미사일">🔧</span>`;
            }).join('');
            ...
            if (building.missileProducing) {
                ...
                document.getElementById('slbmProgressLabel').innerHTML = `<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:4px;">${queueIconsHtml}</div>미사일 제작 중... ${Math.floor(progress * 100)}%`;
...
function renderResources() {
    // 자원 노드 시각화 비활성화 (유저 요청)
    // 자원은 여전히 존재하며 일꾼이 채집 가능
    return;
}
 ...
 * Called once per updateFogOfWar() tick (≈1.5 Hz), NOT every rAF frame.
 ...
 *   3. source-over draw stale-explored cells with 0–0.5 alpha gradual overlay
 */
...
function renderUnits() {
    ...
    gameState.units.forEach((unit, unitId) => {
        ...
        // Face the direction of the last issued command.
        // Keeps the unit pointing that way after it arrives (no frozen/default angle).
        if (unit.commandAngle !== undefined) {
            ctx.rotate(unit.commandAngle);
        }
        ...
        if (unit.type === 'aircraft') {
            ...
            if (battleshipBaseLoaded && battleshipBaseImage) {
                // Use original image dimensions and scale proportionally
                const originalWidth = battleshipBaseImage.width;
                const originalHeight = battleshipBaseImage.height;
                const aspectRatio = originalWidth / originalHeight;
                
                // Scale to fit unit size while maintaining aspect ratio
                // Since image is taller than wide (vertical), use height as reference
                // Scale by 3x for both width and height
                let baseWidth, baseHeight;
                baseHeight = size * 2.2 * 3;
                ...
                // Turret positions matching battleshipbasewcannon.png
                // Assuming turrets are centered horizontally (at centerX)
                const turretPositions = [
                    { x: (centerX - centerX) * imageScaleX, y: (15 - centerY) * imageScaleY },  // Front turret
                    { x: (centerX - centerX) * imageScaleX, y: (60 - centerY) * imageScaleY },  // Middle turret
                    { x: (centerX - centerX) * imageScaleX, y: (70 - centerY) * imageScaleY }   // Rear turret
                ];
                
                // Calculate angle to target for turrets (in world space)
                // Use actual attack target, not movement target
                let targetTurretAngle = unit.turretAngle || 0; // Default to current angle
                let hasAttackTarget = false;
                
                if (unit.attackTargetId) {
                    // Find the actual attack target
                    let attackTarget = null;
                    if (unit.attackTargetType === 'unit') {
                        attackTarget = gameState.units.get(unit.attackTargetId);
                    } else if (unit.attackTargetType === 'building') {
                        attackTarget = gameState.buildings.get(unit.attackTargetId);
                    }
                    
                    // If target exists, calculate angle to it
                    if (attackTarget) {
                        targetTurretAngle = Math.atan2(attackTarget.y - posY, attackTarget.x - posX);
                        hasAttackTarget = true;
                    }
                }
                
                // Ship faces the same angle as the outer ctx.rotate (commandAngle).
                const shipAngle = unit.commandAngle !== undefined ? unit.commandAngle : 0;
                
                // Initialize turret angle if not exists
                if (unit.turretAngle === undefined) {
                    unit.turretAngle = targetTurretAngle;
                }
                
                // Only rotate turret if there's an attack target
                if (hasAttackTarget) {
                    // Smooth turret rotation with speed limit
                    const turretRotationSpeed = 0.08; // Radians per frame (~4.5 degrees) - faster rotation
                    let angleDiff = targetTurretAngle - unit.turretAngle;
                    ...
                    if (Math.abs(angleDiff) > turretRotationSpeed) {
                        unit.turretAngle += Math.sign(angleDiff) * turretRotationSpeed;
                    } else {
                        unit.turretAngle = targetTurretAngle;
                    }
                }
                // If no attack target, keep current angle (don't rotate)
                
                const turretAngle = unit.turretAngle;
                
                if (mainCannonLoaded && mainCannonImage) {
                    const cannonOriginalWidth = mainCannonImage.width;
                    const cannonOriginalHeight = mainCannonImage.height;
                    
                    // Scale turret with same pixel ratio as battleship base
                    ...
                    turretPositions.forEach(pos => {
                        ...
                        // +PI/2 compensates for the -PI/2 ship-body rotation applied in the enclosing ctx.save(),
                        // so that 0 in this context maps to the ship's forward direction in world space.
                        const relativeAngle = turretAngle - shipAngle + Math.PI / 2;
...
function showKillLog(attackerName, defeatedName) {
    ...
    message.innerHTML = `<span class="attacker">${attackerName}</span> 님이 <span class="defeated">${defeatedName}</span> 을(를) 처치했습니다!`;
...
async function resetGame() {
    ...
    if (!confirm('정말로 내 기지를 초기화하시겠습니까?\n모든 유닛, 건물, 자원이 초기화되고\n새로운 랜덤 위치에서 시작합니다.')) {
        return;
    }
...
function connectToGame() {
    ...
    // 화면 전환은 init 이벤트를 받은 후에 하기 (여기서는 하지 않음)
    ...
    socket.on('connect_error', (error) => {
        ...
        document.getElementById('loginScreen').classList.add('active');
        document.getElementById('gameScreen').classList.remove('active');
        document.getElementById('authError').textContent = '연결 실패: 토큰이 유효하지 않습니다.';
        localStorage.removeItem('token');
    });
    ...
    socket.on('gameUpdate', (data) => {
        ...
        data.units.forEach(u => {
            ...
            if (existingUnit) {
                ...
                if (existingUnit.turretAngle  !== undefined) u.turretAngle  = existingUnit.turretAngle;
            } else {
    ...
    socket.on('playerDefeated', (data) => {
        ...
        if (data.userId === gameState.userId && data.respawned) {
            ...
            fogDirty = true;
            minimapDirty = true;
            alert('패배! 모든 건물이 파괴되었습니다. 새로운 위치에서 다시 시작합니다.');
        }
    });
    ...
    socket.on('attackProjectileFired', (data) => {
        ...
        if (isBattleship) {
            // --- Triple-turret salvo ---
            // Compute the ship's facing angle from commandAngle (set client-side when a command is
            // issued). Fall back to the shot direction so it still looks right for AI/remote ships.
            const shooter   = data.shooterId ? gameState.units.get(data.shooterId) : null;
            const shipAngle = (shooter && shooter.commandAngle !== undefined)
                ? shooter.commandAngle
                : Math.atan2(data.targetY - data.fromY, data.targetX - data.fromX);

            // World-space offset of each turret along the ship's main axis.
            // Values are based on the turret pixel positions inside battleshipbase.png scaled to world units;
            // positive = toward the bow (+forward along shipAngle).
            let origH = (battleshipBaseLoaded && battleshipBaseImage) ? battleshipBaseImage.height : 200;
            const baseH     = 60 * 2.2 * 3;          // = 396 world units (matches render code)
            const scaleY    = baseH / origH;
            const centerY   = origH / 2;
            // Turret image Y coords (from render code): 15, 60, 70
            // In image space +y = bow direction; after -PI/2 transform +y_inner maps to +world-x (ship forward)
            const turretImgY = [15, 60, 70];
            const turretOffsets = turretImgY.map(ty => (ty - centerY) * scaleY); // negative = toward bow
            // bow is at bottom of image (+y), so forward in world = +y_inner → (0,ty-centerY) after -PI/2 maps ->(ty-centerY,0)
            // ties out: negative offset = toward stern when bow is at-bottom; flip sign so "front turret" leads
            const cosA = Math.cos(shipAngle);
            const sinA = Math.sin(shipAngle);

            turretOffsets.forEach((offset, i) => {
                // turret image y is measured from top; bow = bottom (+y image), stern = top (-y image).
                // offset is negative for front turret (top of image) → negate so front is forward.
                const worldOffset = -offset;
                attackProjectiles.push({
                    id: `${baseId}-${i}`,
                    fromX: data.fromX + cosA * worldOffset,
                    fromY: data.fromY + sinA * worldOffset,
                    targetX:    data.targetX,
                    targetY:    data.targetY,
